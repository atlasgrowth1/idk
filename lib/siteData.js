...
export function getSiteData(siteName) {
  // Handle arrays (from Next.js router query params)
  if (Array.isArray(siteName)) {
    siteName = siteName[0];
  }
  
  // If no site name is provided or it's an empty string, return default data
  ...
  // Return default data with the site name if site not found
  return {
    "name": "Local Electrician",
